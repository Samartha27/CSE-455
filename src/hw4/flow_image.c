...
{
    int i,j,k;
    int A_i,A_j,D_i,D_j, i_overlap, j_overlap;
    ...
    for ( k = 0;k<im.c;k++){
        for ( j = 0;j<im.h;j++){
            for ( i = 0;i<im.w;i++){
                ...
                if(A_j<0) A_j=-1;
                D_i = i+w;
                if(D_i>=im.w) D_i=im.w-1;
                D_j = j+w;
                if(D_j>=im.h) D_j=im.h-1;

                i_overlap = D_i - A_i;
                j_overlap = D_j - A_j;
                
                v4 = get_pixel(integ,i+w,j+w,k);
                if(A_i>=0) {v3 = get_pixel(integ,i-w-1,j+w,k);}
                if(A_j>=0) {v2 = get_pixel(integ,i+w,j-w-1,k);}
                if((A_i>=0)&&(A_j>=0)) v1 = get_pixel(integ,i-w-1,j-w-1,k);
                value = v1+v4-v3-v2;
                final_value = value/(i_overlap*j_overlap);
...
image time_structure_matrix(image im, image prev, int s)
{
    ...
    for ( y = 0;y<im.h;y++){
        for ( x = 0;x<im.w;x++){
            set_pixel(S,x,y,0,get_pixel(i_x,x,y,0)*get_pixel(i_x,x,y,0));
            set_pixel(S,x,y,1,get_pixel(i_y,x,y,0)*get_pixel(i_y,x,y,0));
            set_pixel(S,x,y,2,get_pixel(i_x,x,y,0)*get_pixel(i_y,x,y,0));
            set_pixel(S,x,y,3,-get_pixel(it,x,y,0)*get_pixel(i_x,x,y,0));
            set_pixel(S,x,y,4,-get_pixel(it,x,y,0)*get_pixel(i_y,x,y,0));
        }}
...
image velocity_image(image S, int stride)
{
    ...
    for(j = (stride-1)/2; j < S.h; j += stride){
        for(i = (stride-1)/2; i < S.w; i += stride){
            ...
            if (Minv.data){
                matrix delta_p = matrix_mult_matrix(Minv,StT);
